...
Returned when you exceed the hourly rate limit (3,600 requests per hour) or your monthly usage quota.
...
**How to fix:** Check the `Retry-After` header for how many seconds to wait before retrying. Monitor the `X-RateLimit-Remaining` and `X-Monthly-Remaining` response headers to track your usage proactively. See [hourly rate limits](/guides/authentication#hourly-rate-limits) and [monthly usage limits](/guides/authentication#monthly-usage-limits) for quota details by plan.
